...
function getNextFriday(date) {
  date.setDate(date.getDate() + (1 + ((11 - date.getDay()) % 7))).toString();
  return date;
}
...
function formatDate(date) {
  ...
  const result = `${month}/${day}/${year}, ${hours === 12 ? 12 : hours % 12}:${minutes}:${seconds} ${hours < 12 && hours >= 0 ? 'AM' : 'PM'}`;
  return result;
}
...
function getWeekNumberByDate(date) {
  ...
  const weekEnd = new Date(date.getFullYear(), 0, 1);
  while (weekEnd.getDay() !== 0) {
    weekEnd.setDate(weekEnd.getDate() + 1);
  }
  let result = yearStart.getDay() <= 4 && yearStart.getDay() !== 0 ? 1 : 0;
  while (weekEnd < date) {
    result += 1;
